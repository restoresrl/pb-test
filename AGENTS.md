...
```text
framework/   PowerScript sources of the test framework, .sru, UTF-8 BOM, CRLF
...
The `.sru` files are the truth; `pbtest.pbl` is a build artefact. Never
commit a PBL.
...
- **Keep the framework dependency order**: report, case, suite, runner.
  Nothing references forward.
